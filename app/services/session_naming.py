"""Auto-name a session from its first message, using a background LLM call."""

import asyncio

from langchain_core.messages import HumanMessage, SystemMessage
from sqlmodel import Session as DBSession, col, update
...
async def _persist_session_name(session_id: str, user_message: str) -> None:
    try:
        result = await llm_service.call(
            [SystemMessage(content=SESSION_TITLE_PROMPT), HumanMessage(content=user_message[:500])],
...
def _claim_session(session_id: str, placeholder: str) -> bool:
    """Atomically claim the naming job so concurrent requests don't double-fire."""
...
def maybe_name_session(session_id: str, session_name: str, messages: list) -> None:
    """Kick off session auto-naming if unnamed. Safe to call on every message."""
    if session_name:
        return
